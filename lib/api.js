...
(function () {
  ...
  async function request(base, path, params) {
    ...
    const data = await pageRequest(url);
  ...
  function actorEntries(raw) {
    ...
    return rows
      ...
      .filter((actor) => actor.actorId && (actor.totalRevenueUsd > 0 || actor.totalCostUsd > 0));
  }
  ...
  self.AAP_API = {
    ...
    rangeData: async (start, end, actorIds) => {
      ...
      // needs the per-Actor daily profit-margin response. Fetch only the top
      // eight Actors across the selected range; the remainder is represented
      // by an "Other actors" stack segment, preserving the account total.
      ...
      const actorMargins = await pooled(tasks, async (task) => ({
        ...
        margin: await req("profit-margin", { month: task.month, actorIds: [task.actor.actorId] }),
      }));
