...
(function () {
  ...
  self.AAP_API = {
    ...
    rangeData: async (start, end, actorIds) => {
      ...
      const actorsByMonth = monthly.map((item) => {
        const seen = new Set();
        return actorEntries(item.actorBreakdown).filter((actor) => !seen.has(actor.actorId) && seen.add(actor.actorId));
      });
      for (const actorsForMonth of actorsByMonth) {
        for (const actor of actorsForMonth) {
      ...
      const tasks = [];
      // Do not ask for daily metrics for an Actor in months where the
      // breakdown says it had no activity. This keeps long ranges bounded
      // without hiding any Actor that can actually appear in a tooltip.
      for (let index = 0; index < monthly.length; index++) {
        for (const actorEntry of actorsByMonth[index]) {
          const actor = ranked.get(actorEntry.actorId);
          if (actor) tasks.push({ index, month: monthly[index].month, actor });
        }
      }
      ...
      const failures = actorMargins.failures || [];
      if (failures.length) {
        const failedActors = failures.map(({ item }) => ({
          actorId: item?.actor?.actorId || null,
          month: item?.month || null,
        }));
        const uniqueActorIds = new Set(failedActors.map(({ actorId }) => actorId).filter(Boolean));
        aggregated.partial = {
          failedCount: uniqueActorIds.size || failures.length,
