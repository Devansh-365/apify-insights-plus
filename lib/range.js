  ...
  function aggregate(months, start, end) {
    ...
    for (const month of months || []) {
      ...
      for (const entry of month?.actorMargins || []) {
        ...
        actorNames[entry.actorId] = entry.name || entry.actorId;
        const marginByDay = entry.margin?.dailyProfitMarginStats || {};
        for (const day of Object.keys(marginByDay)) {
          if (!inRange(day, start, effectiveEnd)) continue;
          const row = actorRow(entry, day);
          if (row) actorByDay.set(`${day}|${entry.actorId}`, row);
        }
      }
    }
    ...
    for (const day of days) {
      ...
      const actors = [...actorByDay.entries()]
  ...
  function group(data, grouping) {
    ...
    for (const day of data?.days || []) {
      ...
      for (const actor of data.actorDaily?.[day] || []) {
        bucket.actorRevenue[actor.actorId] = (bucket.actorRevenue[actor.actorId] || 0) + number(actor.revenue);
      }
    }
    ...
    for (const [key, bucket] of buckets) {
      ...
      daily[key] = {
        ...
        actorRevenue: bucket.actorRevenue,
      };
    }
