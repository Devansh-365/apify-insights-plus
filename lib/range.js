...
(function () {
  ...
  // Combines the existing per-Actor daily response shape for the exact days
  // represented by a tooltip bucket. A week or month can span multiple API
  // months, so callers pass all matching month/Actor detail entries here and
  // get one correctly summed row per Actor.
  function actorRowsForDays(entries, days) {
    ...
    for (const entry of entries || []) {
      ...
      for (const day of entryDays) {
        ...
        existing.margin = existing.revenue ? existing.profit / existing.revenue : null;
      }
    }
